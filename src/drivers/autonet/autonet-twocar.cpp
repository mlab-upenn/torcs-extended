...
void serialData(int signum)
{
    ...
    uint8_t b[21];  // Changed from 12 to 20 for second car  // Changed from 11 to 12 for autoplug 3.0 to include distance metric
    int i;
    ...
    if(car2 == NULL) {
        ...
        b[11] = 0x01;b[12] = 0x01;b[13] = 0x01;b[14] = 0x04;b[15] = 0x05;b[16] = 0x06;b[17] = 0x07;b[18] = 0x08;
    }
    #endif
   else
    {
    ...
    for(i = 0; i <= 19; i++){   // Changed from 10 to 18 for second car // Changed from 9 to 10
        calcChkSum = calcChkSum ^ b[i];
    }
   
    b[20] = calcChkSum;  // changed from 11 to 19 for new distance method // Changed - was b[10] earlier // 

    //if(serialPortWrite(serialfd, b, 15) == -1)
      //perror("Error");
    //cout<<"Bytes Written " << serialPortWrite(serialfd, b, 20);
    serialPortWrite(serialfd, b, 21);  // Changed from 1 for distance // Changed size from 11 to 12
}
...
static void drive(int index, tCarElt* car, tSituation *s)
{
   ...
   else if(index == 1) 
   {
     ...
     // set the values
    // cout<<"Angle 2 value is " << angle2 << endl;
   ...
   #if 1 
   if(dist1 >=  dist2)
      dist_cars2 = abs(dist1-dist2) / 8;
   else
      dist_cars2 = (((int)(curTrack->length) - abs(dist1-dist2))) / 8;  // 
   #endif
