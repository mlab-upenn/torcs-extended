...
static int gear = 0 , gear2 = 0,radarAngle=6, lanePosition;
//uint16_t distance;
 int dist1 = 0, dist2 = 0;
...
{ 
   if  (((pos1 - 3.0) < pos2) && ((pos1 + 3.0) > pos2))
       return 1;
...
static void drive(int index, tCarElt* car, tSituation *s)
{
   ...
   else if(index == 1) 
   {
    ...
    //cout<<"Dist2 value is " <<dist2<<endl;
     ...
     driver[index]->drive(car2, s,index,front_car_Speed,lanePosition);
