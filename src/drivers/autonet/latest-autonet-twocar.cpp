...
void serialData(int signum)
{
    ...
    uint8_t b[28];  // 	Changed from 12 to 15 to send distance as 2 bytes
    int i;
  ...
  if(dist_cars < 160) 
   ...
   if(dist_cars < 160) 
   {
    ...
    if(car2 == NULL) {
        ...
        b[14] = 0x01;b[15] = 0x01;b[16] = 0x01;b[17] = 0x04;b[18] = 0x05;b[19] = 0x06;b[20] = 0x07;b[21] = 0x08;b[22]=0x08;
    }
    #endif
    else
    {
    ...
    #if 1
    b[14] = (uint8_t)car2->_speed_x;
    
    uint16_t rpm2 = car2->_enginerpm;
    b[15] =(uint8_t)((rpm2 & 0xFF00) >> 8);
    b[16] = (uint8_t)rpm2 & 0xFF;
 
    b[17] = (uint8_t)car2->_wheelSpinVel(FRNT_LFT) * car2->_wheelRadius(FRNT_LFT);
    b[18] = (uint8_t)car2->_wheelSpinVel(FRNT_RGT) * car2->_wheelRadius(FRNT_RGT);
    b[19] = (uint8_t)car2->_wheelSpinVel(REAR_LFT) * car2->_wheelRadius(REAR_LFT);
    b[20] =(uint8_t)car2->_wheelSpinVel(REAR_RGT) * car2->_wheelRadius(REAR_RGT);

    b[21] = (uint8_t)(car2->_yaw_rate * 10);
    b[22] = 10;//(uint8_t)dist2;
    #endif
    }
   ...
   if(car2 != NULL)
    ...
    serialPortWrite(serialfd, b, 28);  // Changed from 12 to 15 for sending distance as 10 bytes
}
