...
void serialData(int signum)
{
    ...
    uint8_t b[12];  // Changed from 12 to 20 for second car  // Changed from 11 to 12 for autoplug 3.0 to include distance metric
    int i;
   ...
   // }
    ...
    for(i = 0; i <= 10; i++){   // Changed from 10 to 18 for second car // Changed from 9 to 10
        calcChkSum = calcChkSum ^ b[i];
    }
   
    b[11] = calcChkSum;  // changed from 11 to 19 for new distance method // Changed - was b[10] earlier // 

    //if(serialPortWrite(serialfd, b, 15) == -1)
      //perror("Error");
    //cout<<"Bytes Written " << serialPortWrite(serialfd, b, 20);
    serialPortWrite(serialfd, b, 12);  // Changed from 1 for distance // Changed size from 11 to 12
}
