...
void serialData(int signum)
{
    ...
    uint8_t b[15];  // 	Changed from 12 to 15 to send distance as 2 bytes
    int i;
    ...
    uint16_t start_distance = start_dist;
    b[12] =(uint8_t)((start_distance & 0xFF00) >> 8); // Lower 8 bytes of start distance of Car1 
    b[13] = (uint8_t)(start_distance & 0xFF);         // Higher 8 bytes of start distance of Car1 
    ...
    serialPortWrite(serialfd, b, 15);  // Changed from 12 to 15 for sending distance as 10 bytes
}
...
static void drive(int index, tCarElt* car, tSituation *s)
{
   ...
   if(index == 0) 
   {
     ...
     start_dist = car->_trkPos.seg->lgfromstart;
   ...
   else if(index == 1) 
   {
    ...
    //cout<<"Dist2 value is " <<dist2<<endl;
     ...
     //cout<<"Car X and Y positions are " << car2->_pos_X<<"\t"<<car2->_pos_Y<<endl;
