...
void Driver::initTrack(tTrack* t, void *carHandle, void **carParmHandle, tSituation *s)
{
	...
	*carParmHandle = GfParmReadFile(buffer, GFPARM_RMODE_STD);
	...
	PASS_DIST = GfParmGetNum(*carParmHandle, BT_SECT_PASS, BT_ATT_DIST, (char*)NULL, 10.0f);

	ACCEL_DEC = GfParmGetNum(*carParmHandle, BT_SECT_PASS, BT_ATT_ACCEL, (char*)NULL, 0.5f);
}
...
void Driver::drive(tSituation *s)
{
	...
	if (isStuck()) {
		...
		if (car->_brakeCmd == 0.0f) {
			car->_accelCmd = filterPass(filterTCL(filterTrk(filterOverlap(getAccel()))));
		} else {
			car->_accelCmd = 0.0f;
  		}
	...
	FILE * pFile;
   	...
   	char name[256];
   	sprintf(name,"/usr/local/share/games/torcs/telemetry/%d.txt",INDEX);
   	

    pFile = fopen (name,"a");
    fprintf (pFile, "X: %f Y: %f Speed %f\n",x1,y1,currentspeed);
...
/* set repeated passing margin by limiting acceleration */
float Driver::filterPass(float accel)
{
	int i;
	for (i = 0; i < opponents->getNOpponents(); i++) {
		if (opponent[i].getDistance() < -PASS_DIST) {
			return accel*ACCEL_DEC;
		}
	}
}
