...
*/
	...
	fp = fopen("/usr/local/share/games/torcs/datafile.txt","a");
	if(fp == NULL)
	{
	  //perror("Fucked");
	}
	...
	long int x, Y;
	int v;
	long int new_time, temp;
	long int temp_time;
	
/*	if(time_flag == 0)
	 {
	  time(&t0);	//returns the present time in sec
	  time_flag = 1;
	  x = 100;
	 }
	 else if(time_flag == 1)
